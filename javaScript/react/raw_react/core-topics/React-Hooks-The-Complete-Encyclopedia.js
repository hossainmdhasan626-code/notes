...
/**
 * ১৩. useId (অ্যাক্সেসিবিলিটি এবং ইউনিক আইডি মাস্টার) - [React 18+]
 * -----------------------------------------------------------
 * IMPORT: import { useId } from 'react';
 * CONVENTION: const id = useId();
 */
...
/**
 * ১৩.২. Database ID থাকতে useId কেন লাগবে? (হাসানের প্রশ্ন):
 * ---------------------------------------------------------
 * | বৈশিষ্ট্য          | Database ID (যেমন: _id)        | React useId                   |
 * | :---              | :---                           | :---                           |
 * | উৎস               | ব্যাকএন্ড/ডাটাবেস থেকে আসে।      | রিয়্যাক্ট ইঞ্জিন থেকে আসে।       |
 * | ব্যবহার           | key={item.id} (ম্যাপের জন্য)।   | id={id} (HTML লেবেলের জন্য)।   |
 * | ইউনিক থাকা        | ডাটা চেনার জন্য ইউনিক।          | UI এলিমেন্ট চেনার জন্য ইউনিক।   |
 * | সমস্যা             | এক-ই পেজে ২ বার ব্যবহার করলে কনফ্লিক্ট হয়। | ১০০ বার ব্যবহার করলেও কনফ্লিক্ট হয় না। |
 */
...
/**
 * ১৩.৪. কোথায় কোথায় ব্যবহার করবে? (Practical Use Cases):
 * -------------------------------------------------
 * ১. Form Accessibility: <label htmlFor={id}> এবং <input id={id}> লিঙ্ক করার জন্য।
 * ২. Reusable Components: যখন এক-ই ইনপুট কম্পোনেন্ট পেজে বারবার ব্যবহার করা হয়।
 * ৩. Aria Attributes: aria-describedby বা aria-labelledby-তে ইউনিক আইডি দিতে।
 */
...
function App() {
  return (
    <div>
      {/* এক-ই কম্পোনেন্ট ২ বার ব্যবহার করলেও আইডি কনফ্লিক্ট হবে না */}
      <MyInput label="Header Login Email" />
      <MyInput label="Footer Login Email" />
    </div>
  );
}
...
/**
 * ১৫. DOM Mutation এবং useInsertionEffect (টাইমিং মাস্টার নোট)
 * ---------------------------------------------------------
 * ১৫.১. DOM Mutation কী? (সহজ ভাষায়):
 * - ব্রাউজারের আসল HTML স্ট্রাকচারে কোনো পরিবর্তন করাকেই (যোগ করা, মুছে ফেলা বা টেক্সট বদলানো)
 * DOM Mutation বলে।
 * - রিঅ্যাক্ট যখন Virtual DOM-এর হিসাব শেষ করে আসল ব্রাউজারের DOM-কে আপডেট করে,
 * তখনই এই মিউটেশন ঘটে।
 */

/**
 * ১৫.২. রিঅ্যাক্ট রেন্ডারিং-এর ৩টি প্রধান ধাপ (The 3 Phases):
 * ----------------------------------------------------
 * ১৫.২.১. Render Phase: রিঅ্যাক্ট হিসাব করে দেখে কী কী বদলাতে হবে (ভার্চুয়াল ডম)।
 * ১৫.২.২. Commit Phase: এখানেই 'DOM Mutation' ঘটে। রিঅ্যাক্ট ব্রাউজারের আসল এলিমেন্টগুলো আপডেট করে।
 * ১৫.২.৩. Paint Phase: ব্রাউজার স্ক্রিনে নতুন পরিবর্তনগুলো ইউজারের চোখে দেখায় (UI)।
 */

/**
 * ১৫.৩. হুকগুলোর টাইমিং তুলনা (The Grand Battle):
 * -------------------------------------------
 * | হুক নাম               | কখন রান করে? | মূল কাজ |
 * | :---                 | :--- | :--- |
 * | **useInsertionEffect** | DOM Mutation-এর **ঠিক আগে**। | স্টাইল ইনজেক্ট করা (CSS-in-JS)। |
 * | **DOM Mutation** | (এই ধাপে ব্রাউজারের HTML বদলে যায়)। | - |
 * | **useLayoutEffect** | DOM Mutation-এর **পরে**, কিন্তু Paint-এর **আগে**। | লেআউট বা মাপজোখ (Size, Position) হিসাব করা। |
 * | **useEffect** | ব্রাউজার সবকিছু **Paint করার পরে**। | API কল, সাবস্ক্রিপশন বা সাধারণ কাজ। |
 */

/**
 * ১৫.৪. useInsertionEffect (CSS-in-JS এর জন্য স্পেশাল) - [React 18+]
 * -----------------------------------------------------------
 * - কাজ: রান-টাইমে <style> ট্যাগ ব্রাউজারের <head>-এ ইনসার্ট করা।
 * - কেন জরুরি: যদি স্টাইলগুলো DOM মিউটেশনের আগে ইনজেক্ট না হয়, তবে ব্রাউজারকে বারবার
 * স্টাইল রি-ক্যালকুলেট করতে হয়, যা অ্যাপকে স্লো করে দেয়।
 */

// --- ১৫.৫. প্রাকটিক্যাল উদাহরণ (স্টাইল ইনজেকশন) ---

import { useInsertionEffect } from "react";

function DynamicButton({ color }) {
  useInsertionEffect(() => {
    // ১. ডাইনামিক স্টাইল তৈরি
    const style = document.createElement("style");
    style.textContent = `
      .hasan-dynamic-btn { background: ${color}; color: white; padding: 12px; }
    `;

    // ২. DOM মিউটেশনের আগেই স্টাইল ইনজেক্ট করা
    document.head.appendChild(style);

    return () => document.head.removeChild(style);
  }, [color]);

  return <button className="hasan-dynamic-btn">ক্লিক করো</button>;
}

/**
 * হাসানের প্রো-টিপস (সতর্কতা):
 * --------------------------
 * ১. এই হুকের ভেতর থেকে কখনো 'State' আপডেট করার চেষ্টা করবে না।
 * ২. এটি সাধারণ অ্যাপ ডেভেলপমেন্টে ব্যবহার করা "বোকামি"। এটি শুধু লাইব্রেরি মেকারদের জন্য।
 * ৩. ইন্টারভিউতে বলবে: "এটি স্টাইল ইনজেকশনের পারফরম্যান্স ঠিক রাখার জন্য মিউটেশনের আগে রান করে।"
 */

/**
 * ১৬. use API (The Modern Resource Handler) - [React 19 / 18.3+]
 * -----------------------------------------------------------
 * IMPORT: import { use } from 'react';
 * CONVENTION: const data = use(promise); 
 * OR: const value = use(context);
 */

/**
 * ১৬.১. use-এর বহুমুখী কাজ কী? (The Core Job):
 * --------------------------------------
 * - এটি 'Promise' (ডাটা ফেচিং) অথবা 'Context' (গ্লোবাল স্টেট) থেকে সরাসরি মান পড়ার জন্য ব্যবহৃত হয়।
 * - বৈপ্লবিক বৈশিষ্ট্য: এটিই একমাত্র হুক যা 'if' কন্ডিশন বা 'for' লুপের ভেতরেও ব্যবহার করা যায়!
 * - এটি রিঅ্যাক্টকে বলে: "ডাটা না আসা পর্যন্ত অপেক্ষা করো এবং Suspense লোডিং দেখাও।"
 */

/**
 * ১৬.২. কেন এটি বৈপ্লবিক? (The Innovation):
 * -----------------------------------
 * - Data Fetching: আগে useEffect + useState লাগত, এখন শুধু এক লাইনের 'use(promise)' যথেষ্ট।
 * - Conditional Context: আগে useContext কম্পোনেন্টের শুরুতে দিতেই হতো, এখন 'if' এর ভেতর প্রয়োজন বুঝে কন্টেক্সট রিড করা যায়।
 * - Clean Code: এটি বয়লারপ্লেট কোড কমিয়ে দেয় এবং রেন্ডারিং প্রসেসকে আরও স্মুথ করে।
 */

/**
 * ১৬.৩. কোথায় কোথায় ব্যবহার করবে? (Practical Use Cases):
 * -------------------------------------------------
 * ১৬.৩.১. প্রমিস থেকে ডাটা রিড করতে: সরাসরি API কল বা ডাটা ফেচিংয়ের সময়।
 * ১৬.৩.২. শর্তসাপেক্ষে কন্টেক্সট ব্যবহার: যখন কোনো নির্দিষ্ট কন্ডিশনে গ্লোবাল ডাটা দরকার হয়।
 * ১৬.৩.৩. সার্ভার-টু-ক্লায়েন্ট ডাটা: সার্ভার কম্পোনেন্ট থেকে পাঠানো প্রমিস ক্লায়েন্ট সাইডে আনলক করতে।
 */

// --- ১৬.৪. পরিপূর্ণ উদাহরণ (হাসানের স্মার্ট ডাটা ও কন্টেক্সট হ্যান্ডলার) ---

import { use, Suspense } from 'react';

// ১৬.৪.১. একটি প্রমিস (কম্পোনেন্টের বাইরে ডিফাইন করা ভালো)
const messagePromise = fetch('/api/message').then(res => res.json());

function MessageComponent({ showDetails }) {
  // ১৬.৪.২. সরাসরি প্রমিস থেকে ডাটা পড়া (useEffect ছাড়াই!)
  const data = use(messagePromise);

  // ১৬.৪.৩. কন্ডিশনাল কন্টেক্সট রিড করা (আগে এটি অসম্ভব ছিল!)
  if (showDetails) {
    const theme = use(ThemeContext); 
    return <div style={{ color: theme.color }}>{data.text}</div>;
  }
...
function App() {
  return (
    // ১৬.৪.৪. use হুক যখন ওয়েট করবে, তখন এটি দেখাবে
    <Suspense fallback={<p>লোড হচ্ছে...</p>}>
      <MessageComponent showDetails={true} />
    </Suspense>
  );
}

/**
 * ১৬.৫. হাসান'স প্রো-টিপস (সতর্কতা):
 * ----------------------------
 * ১৬.৫.১. বোকামি ১: রেন্ডারিংয়ের ভেতরে সরাসরি fetch করা (যেমন: use(fetch(...)))। 
 * এতে ইনফিনিট লুপ হতে পারে। প্রমিসটি সবসময় বাইরে বা ক্যাশ (cache) করে রাখতে হবে।
 * ১৬.৫.২. এটি ব্যবহারের সময় অবশ্যই 'Suspense' বা 'Error Boundary' ব্যবহার করতে হবে।
 * ১৬.৫.৩. এটি useEffect-এর বিকল্প হিসেবে পেজ লোড হওয়ার সাথে সাথে ডাটা ফেচ করতে সেরা।
 */
